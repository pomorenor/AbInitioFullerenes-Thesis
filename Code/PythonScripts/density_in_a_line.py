...
FileName_1D = cubeFileName + ".Z_line.dat"
File_1D = open(FileName_1D, "w")

# choose the position of the line in real space
x0 = 0.0
y0 = 0.0

# convert to grid indices
ix = int((x0 - origin[0]) / steps[0][0])
iy = int((y0 - origin[1]) / steps[1][1])

# scan along z
for k in range(0, n_points[2]):
    z = origin[2] + steps[2][2]*k
    rho = grid[ix, iy, k]
    File_1D.write("%.6f %.10e\n" % (z, rho))

File_1D.close()
    
FileName_1D = cubeFileName + ".radial.dat"
File_1D = open(FileName_1D, "w")

for k in range(0, n_points[2]):
    z = origin[2] + steps[2][2]*k
    r = abs(z)  # if scanning along z through origin
    rho = grid[ix, iy, k]
    File_1D.write("%.6f %.10e\n" % (r, rho))

FileName_1D = cubeFileName + ".X_line.dat"
File_1D = open(FileName_1D, "w")

# choose position of the line
y0 = 0.0
z0 = 0.0

# convert to grid indices
iy = int((y0 - origin[1]) / steps[1][1])
iz = int((z0 - origin[2]) / steps[2][2])

# scan along x
for i in range(0, n_points[0]):
    x = origin[0] + steps[0][0]*i
    rho = grid[i, iy, iz]
    File_1D.write("%.6f %.10e\n" % (x, rho))
...
FileName_1D = cubeFileName + ".Y_line.dat"
File_1D = open(FileName_1D, "w")
